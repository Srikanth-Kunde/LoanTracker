...
*   **Auto-Gen Stability & Dependencies**: Fortified the generation preview to correctly react to `settings` and `interest_rate_rules` changes, removed an invalid Supabase-session dependency from `fetchFinancials`, corrected the liveness check that was producing false `401 Unauthorized` alarms, and added non-blocking loading states (`isGenerating`).
*   **Settings Persistence Integrity**: Resolved a race condition in the `SettingsContext` initialization that could lead to remote settings (like the dynamic rate schedule) being accidentally overwritten by local defaults during app startup.
*   **Importer Pivot Logic**: Replaced fixed-index parsing with dynamic keyword pivoting (searching for "Loan" or "Payment") and implemented "₹" symbol stripping to handle inconsistent spreadsheet pastes.
*   **Financial Mapping**: Enhanced `FinancialContext.tsx` with explicit repayment `entry_type` persistence plus robust audit-log capture.
