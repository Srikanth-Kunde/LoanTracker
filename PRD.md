...
*   **Auto-Gen Stability & Dependencies**: Fortified the generation preview to correctly react to `settings` and `interest_rate_rules` changes, added non-blocking loading states (`isGenerating`), and enhanced batch validation to provide descriptive, period-specific error messages.
...
*   **Importer Pivot Logic**: Replaced fixed-index parsing with dynamic keyword pivoting (searching for "Loan" or "Payment") to handle inconsistent column orders and currency symbols (e.g., ` ₹ 50,000`) in pasted data.
