...
*   **Defense-in-Depth Security (v1.4.5)**: Access control is enforced at both the UI level and the **Internal Logic level**. Administrative tools are strictly isolated using guard clauses.
...
*   **Zero-Balance Gap Handling**: Fixed auto-generation to correctly calculate interest from loan start date, only skipping periods where the principal balance was actually zero. Previously, the code incorrectly skipped all periods before a top-up, missing interest that should have been charged for periods with principal balances.
...
*   **Refined Auto-Gen & Liveness**: Resolved the `At is not a function` (TypeError) by removing minified logger dependencies, fixed the `fetchFinancials` ReferenceError, and silenced `401 Unauthorized` console noise by replacing root-level REST or HEAD liveness probes with authenticated table-level GET pings.
...
*   **Principal Subtraction Fallback (v1.1.1)**: Hardened the interest calculation engine (`getSpecialLoanOutstandingFromEvents` and `buildLoanLedger`) to correctly detect principal reductions even when explicit `principal_paid` fields are missing (falling back to `amount - interestPaid`). This resolves the "117-month overflow" and correctly honors zero-balance gaps in historical ledgers.
*   **Hardened Auto-Gen Persistence (v1.1.1)**: Redesigned the "Apply Auto-Fix" flow to snapshot preview records, perform an explicit background cleanup, and provide a clear **Success Alert** once the database write is confirmed.
