...
*   **Auto-Gen Stability & Dependencies**: Fortified the generation preview to correctly react to `settings` and `interest_rate_rules` changes, resolved a `fetchFinancials` ReferenceError, and added non-blocking loading states (`isGenerating`).
...
*   **Financial Mapping**: Enhanced `FinancialContext.tsx` with explicit `entryType` mapping for and robust audit log capture.
